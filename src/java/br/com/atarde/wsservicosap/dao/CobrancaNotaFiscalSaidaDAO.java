...
public class CobrancaNotaFiscalSaidaDAO {
    ...
    private void inserirInterface(CobrancaNotaFiscalSaidaLinhaModel model, TSDataBaseBrokerIf broker) throws TSApplicationException {
        ...
        model.setInterfaceId(broker.getSequenceNextValue("cobranca_nff_saida_linha_id_seq"));

        broker.setSQL("INSERT INTO COBRANCA_NFF_SAIDA_LINHA (ID, ITEM,QUANTIDADE, VALOR, CODIGO_IMPOSTO, COBRANCA_NFF_SAIDA_ID) VALUES (?,?,?,?,?,?)",
                model.getInterfaceId(),
    ...
    public CobrancaNotaFiscalSaidaModel obterIdExternoInterface(CobrancaNotaFiscalSaidaModel model) {
        ...
        broker.setSQL("SELECT ID FROM COBRANCA_NFF_SAIDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresaModel().getId());
