...
public class EasyclassNotaFiscalSaidaDAO {
    ...
    private void inserirInterface(EasyclassNotaFiscalSaidaLinhaModel model, TSDataBaseBrokerIf broker) throws TSApplicationException {
        ...
        model.setInterfaceId(broker.getSequenceNextValue("easyclass_nff_saida_linha_id_seq"));

        broker.setSQL("INSERT INTO EASYCLASS_NFF_SAIDA_LINHA (ID, ITEM,QUANTIDADE,VALOR_UNITARIO,VALOR,CODIGO_IMPOSTO,EASYCLASS_NFF_SAIDA_ID,U_CMXCOL,U_AREA,U_QUANTIDADE_INSERCOES,U_TOTAL_CMXCOL,U_VALOR_UNITARIO, CST_COFINS, CST_ICMS, CST_IPI, CST_PIS, UTILIZACAO_ID, DEPOSITO_ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                model.getInterfaceId(),
    ...
    public EasyclassNotaFiscalSaidaModel obterIdExternoInterface(EasyclassNotaFiscalSaidaModel model) {
        ...
        broker.setSQL("SELECT ID FROM EASYCLASS_NFF_SAIDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresaModel().getId());
