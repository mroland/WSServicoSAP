...
public class ClassificadosContratoNotaFiscalSaidaDAO {
    ...
    private void inserirInterface(ClassificadosContratoNotaFiscalSaidaLinhaModel model, TSDataBaseBrokerIf broker) throws TSApplicationException {
        ...
        broker.setSQL("INSERT INTO CLASSIFICADOS_CONTRATO_NFF_SAIDA_LINHA (ID,ITEM,QUANTIDADE,VALOR_UNITARIO,VALOR,CODIGO_IMPOSTO,CLASSIFICADOS_CONTRATO_NFF_SAIDA_ID,U_CMXCOL,U_AREA,U_QUANTIDADE_INSERCOES,U_TOTAL_CMXCOL,U_VALOR_UNITARIO, UTILIZACAO_ID, DEPOSITO_ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                model.getInterfaceId(),
    ...
    public ClassificadosContratoNotaFiscalSaidaModel obterIdExternoInterface(ClassificadosContratoNotaFiscalSaidaModel model) {
        ...
        broker.setSQL("SELECT ID FROM CLASSIFICADOS_CONTRATO_NFF_SAIDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresaModel().getId());
