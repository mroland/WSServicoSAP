...
public class ClassificadosExecucaoPedidoVendaDAO {
    ...
    private void inserirInterface(ClassificadosExecucaoNotaFiscalSaidaLinhaModel model, TSDataBaseBrokerIf broker) throws TSApplicationException {
        ...
        model.setInterfaceId(broker.getSequenceNextValue("classificados_execucao_pedidovenda_linha_id_seq"));

        broker.setSQL("INSERT INTO CLASSIFICADOS_EXECUCAO_PEDIDOVENDA_LINHA (ID, ITEM,QUANTIDADE,VALOR,CODIGO_IMPOSTO,CLASSIFICADOS_EXECUCAO_PEDIDOVENDA_ID, UTILIZACAO_ID) VALUES (?,?,?,?,?,?,?)",
                model.getInterfaceId(),
    ...
    public ClassificadosExecucaoNotaFiscalSaidaModel obterIdExternoInterface(ClassificadosExecucaoNotaFiscalSaidaModel model) {
        ...
        broker.setSQL("SELECT ID FROM CLASSIFICADOS_EXECUCAO_PEDIDOVENDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresaModel().getId());
