 ...
 * @author mroland
 */
public class FotografiaPedidoVendaDAO {

    public FotografiaPedidoVendaDAO() {
    }
    ...
    private void inserirInterface(FotografiaNotaFiscalSaidaLinhaModel model, TSDataBaseBrokerIf broker) throws TSApplicationException {
        ...
        model.setInterfaceId(broker.getSequenceNextValue("fotografia_pedidovenda_linha_id_seq"));

        broker.setSQL("INSERT INTO FOTOGRAFIA_PEDIDOVENDA_LINHA (ID, ITEM,QUANTIDADE,VALOR_UNITARIO,VALOR,CODIGO_IMPOSTO,FOTOGRAFIA_PEDIDOVENDA_ID,CFOP, CST_COFINS,CST_ICMS,CST_IPI,CST_PIS,DETALHE_ITEM, UTILIZACAO_ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                model.getInterfaceId(),
    ...
    public FotografiaNotaFiscalSaidaModel obterIdExternoInterface(FotografiaNotaFiscalSaidaModel model) {
        ...
        broker.setSQL("SELECT ID FROM FOTOGRAFIA_PEDIDOVENDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresaModel().getId());
