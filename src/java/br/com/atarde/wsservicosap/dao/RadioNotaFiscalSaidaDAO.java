...
public class RadioNotaFiscalSaidaDAO {
    ...
    public RadioNotaFiscalSaidaModel obterIdExternoInterface(RadioNotaFiscalSaidaModel model) {
        ...
        broker.setSQL("SELECT ID FROM RADIO_NFF_SAIDA WHERE ID_EXTERNO = ? AND EMPRESA_ID = ?", model.getIdExterno(), model.getEmpresaModel().getId());
    ...
    private void inserirInterface(RadioNotaFiscalSaidaLinhaModel model, TSDataBaseBrokerIf broker) throws TSApplicationException {
        ...
        model.setInterfaceId(broker.getSequenceNextValue("radio_nff_saida_linha_id_seq"));

        broker.setSQL("INSERT INTO RADIO_NFF_SAIDA_LINHA (ID, ITEM, QUANTIDADE, VALOR, CODIGO_IMPOSTO, RADIO_NFF_SAIDA_ID, CST_COFINS, CST_ICMS, CST_IPI, CST_PIS, U_SECUNDAGEM, DESCRICAO, DEPOSITO_ID, UTILIZACAO_ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                model.getInterfaceId(),
